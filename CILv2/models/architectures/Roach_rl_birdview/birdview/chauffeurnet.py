...
from CILv2.models.architectures.Roach_rl_birdview.obs_manager import ObsManagerBase
from CILv2.models.architectures.Roach_rl_birdview.utils.traffic_light import TrafficLightHandler
from CILv2.models.architectures.Roach_rl_birdview import run_stop_sign
...
class ObsManager(ObsManagerBase):
    ...
    def _trace_route_to_global_target(self, target_loc):
        global_route = []
        current_location = self._ego_vehicle.get_location()
        for tt in target_loc:
            next_target_location = tt
            route_trace = grp.trace_route(current_location, next_target_location)
    ...
    def attach_ego_vehicle(self, ego_vehicle, route_plan=None):
        self._ego_vehicle = ego_vehicle
        self._world = self._ego_vehicle.get_world()
        self._global_route = route_plan
    ...
    def _get_stops(criteria_stop):
        ...
        if (stop_sign is not None) and (not criteria_stop._stop_completed):
            ...
            stops = [(carla.Transform(trans.location, trans.rotation), bb_loc, bb_ext, stop_sign)]
        return stops
    ...
    def get_observation(self):
        ...
        #vehicles_list = [ve for ve in vehicles_list if ve.id != self._ego_vehicle.id]
        #walkers_list = self._world.get_actors().filter('*walker*')
        ...
        route_in_pixel = np.array([[self._world_to_pixel(wp.location)]
        ...
        # ev_mask
        ev_mask = self._get_mask_from_actor_list([(ev_transform, ev_bbox.location, ev_bbox.extent, self._ego_vehicle)], M_warp)
    ...
    def _get_mask_from_actor_list(self, actor_list, M_warp):
        mask = np.zeros([self._width, self._width], dtype=np.uint8)
        for actor_transform, bb_loc, bb_ext, actor in actor_list:
        ...
        for bbox in bbox_list:
            ...
            if is_within_distance:
                ...
                if actor_list is not None:
                    if len(actor_list) == 1:
                        actor_obj = actor_list[0]
                    else:
                        for actor in actor_list:
                            bbox_loc_center = actor.bounding_box
                            if bbox_loc_center.contains(bbox.location, actor.get_transform()):
                                actor_obj = actor
                            elif bbox.location.distance(actor.get_transform().location)<2.0:
                                actor_obj = actor
                            else:
                                print('bbox does not match! ',bbox.location.distance(actor.get_transform().location))


                actors.append((carla.Transform(bbox.location, bbox.rotation), bb_loc, bb_ext, actor_obj))
        return actors
