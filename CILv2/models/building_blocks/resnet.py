...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, batch_shape=[1, 3, 300, 400]):
        self.inplanes = 64
        ...
        self.fc = nn.Linear(self.get_backbone_output_shape(batch_shape)[-1][-1], num_classes)

        #self.bn2 = nn.BatchNorm2d(num_classes)
        #self.layernorm = nn.LayerNorm(num_classes, eps=1e-5)
        #self.relu = nn.ReLU(inplace=True)
    ...
    def get_layers_features(self, x):
        ...
        all_layers = [x0, x1, x2, x3, x4, x.shape]
        return all_layers
