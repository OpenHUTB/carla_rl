    ...
    if train:
        ...
        while timestep < total_timesteps:
            ...
            if episode % 10 == 0:
                ...
                chkt_file_nums = len(next(os.walk(f'checkpoints/PPO/{town}'))[2])  # 获取保存的检查点目录下文件的数量
                if chkt_file_nums != 0:  # 获取最后一个文件的编号
                    chkt_file_nums -= 1
                chkpt_file = f'checkpoints/PPO/{town}/checkpoint_ppo_' + str(chkt_file_nums) + '.pickle'
                # 更新检查点元数据
                data_obj = {'cumulative_score': cumulative_score, 'episode': episode, 'timestep': timestep,
                            'action_std_init': action_std_init}
                with open(chkpt_file, 'wb') as handle:  # 以二进制写入模式打开文件
                    pickle.dump(data_obj, handle)  # 使用pickle模块序列化保存数据
